...
async function run() {
  // Supabase Auth Admin getUserById or just list all with pagination
  let allUsers = [];
  let page = 1;
  while (true) {
  ...
  const u = allUsers.find(u => u.email === 'minhtam1@vicedu.com');
  console.log('User:', u ? u.raw_user_meta_data : 'Not Found');
}
run();
