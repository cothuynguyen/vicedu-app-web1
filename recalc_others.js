...
async function run() {
  console.log("Fetching all Việt Trì 2 and Dân Hòa students...");
  const { data: students, error } = await supabase.from('students')
    .select('*')
    .in('branch_id', ['Việt Trì 2', 'Dân Hòa']);
    ...
    if (attendance && attendance.length > 0) {
      ...
      if (sessions && sessions.length > 0) {
        ...
        for (const att of attendance) {
          ...
          // Compute cost for this session based on hourly rate
  ...
  console.log(`\nDONE. Processed ${totalProcessed} students for Việt Trì 2 and Dân Hòa.`);
}
