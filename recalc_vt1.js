...
async function run() {
  console.log("Fetching all Việt Trì 1 students...");
  const { data: students, error } = await supabase.from('students').select('*').ilike('branch_id', '%việt trì 1%');
  ...
  for (const stu of students) {
    ...
    await supabase.from('students').update({
      ...
      remaining_cost: total_registered_cost // Assuming they haven't learned anything yet, remaining cost is full
    }).eq('id', stu.id);
  ...
  console.log(`\nDONE. Processed ${totalProcessed} students for Việt Trì 1.`);
}
